...
using inferTensor = DynTensor<infer_t, std::extents<size_t, idxs...>, MemoryBuffer::Allocator<infer_t, 32>>;

template <size_t... idxs> requires ( sizeof...(idxs) > 0 )
using inferTensorView = TensorBase<infer_t, std::extents<size_t, idxs...>>;
...
void BroadcastConvolve( 
  const TensorBase<infer_t, E1>& input,
  const TensorBase<infer_t, E2>& weights,
  const TensorBase<infer_t, E3>& bias,
  TensorBase<infer_t, E4>& out  
) {

  std::array<infer_t, E4::static_extent(1)> membuf;
  inferTensorView<E4::static_extent(1)> partial{ std::span(membuf) };
...
void FeatureExtractor( const inferTensorView<3, 128>& input, inferTensorView<128, 32>& out ) {
  /** -----------------------------------------------------
  ...
  std::array<std::byte, 128 * 64 * sizeof(infer_t) + 256> memory1;
  MemoryBuffer buf1( std::span{memory1} );
  auto allocator1 = buf1.get_allocator<infer_t, 32>();

  std::array<std::byte, 128 * 64 * sizeof(infer_t) + 256> memory2;
  MemoryBuffer buf2( std::span{memory2} );
  auto allocator2 = buf2.get_allocator<infer_t, 32>();

  inferTensor<64, 128> conv_out_layer1(allocator1);  
  inferTensorView<64, 3, 5> conv_weights_layer1{ std::span<infer_t, 64 * 3 * 5>(weights_conv_1d_l1, 64 * 3 * 5) };

  auto slice = conv_weights_layer1[0,0, 0];
  using demo = decltype(slice);

  inferTensorView<64> conv_bias_weights_layer1{ std::span<infer_t, 64>(weights_conv_1d_bias_l1, 64) };

  BroadcastConvolve<2>( input, conv_weights_layer1, conv_bias_weights_layer1, conv_out_layer1 );
   
  inferTensorView<4, 64> bn_weights_layer1{ std::span<infer_t, 4 * 64>( weights_bn_1d_l1, 4 * 64 ) };
  ...
  inferTensor<64, 64> mp_out_layer1(allocator2);
  ...
  inferTensorView<128, 64, 5> conv_weights_layer2( std::span<infer_t, 128 * 64 * 5>( weights_conv_1d_l2, 128 * 64 * 5 ) );
  inferTensorView<128> conv_bias_weights_layer2{ std::span<infer_t, 128>(weights_conv_1d_bias_l2, 128) };
  inferTensor<128, 64> conv_out_layer2( allocator1 );

  BroadcastConvolve<2>( mp_out_layer1, conv_weights_layer2, conv_bias_weights_layer2, conv_out_layer2 );

  inferTensorView<4, 128> bn_weights_layer2( std::span<infer_t, 4 * 128>( weights_bn_1d_l2, 4 * 128 ) );
  ...
  inferTensor<128, 32> mp_out_layer2(allocator2);
  ...
  inferTensorView<128, 128, 3> conv_weights_layer3( std::span<infer_t, 128 * 128 * 3>( weights_conv_1d_l3, 128 * 128 * 3 ) );
  inferTensorView<128> conv_bias_weights_layer3{ std::span<infer_t, 128>(weights_conv_1d_bias_l3, 128) };
  inferTensor<128, 32> conv_out_layer3( allocator1 );

  BroadcastConvolve<1>( mp_out_layer2, conv_weights_layer3, conv_bias_weights_layer3, conv_out_layer3 );

  inferTensorView<4, 128> bn_weights_layer3( std::span<infer_t, 4 * 128>( weights_bn_1d_l3, 4 * 128 ) );
  ...
  inferTensorView<128, 128, 3> conv_weights_layer4( std::span<infer_t, 128 * 128 * 3>( weights_conv_1d_l4, 128 * 128 * 3 ) );
  inferTensorView<128> conv_bias_weights_layer4{ std::span<infer_t, 128>(weights_conv_1d_bias_l4, 128) };
  inferTensor<128, 32> conv_out_layer4(allocator2);

  BroadcastConvolve<1>( conv_out_layer3, conv_weights_layer4, conv_bias_weights_layer4, conv_out_layer4 );

  inferTensorView<4, 128> bn_weights_layer4( std::span<infer_t, 4 * 128>( weights_bn_1d_l4, 4 * 128 ) );
...
void ClassifierHead(const inferTensorView<128, 32>& input, inferTensorView<11>& out ) {
  
  // Same idea as FeatureExtractor; preallocate the stack memory we will use for temporaries.
  std::array<std::byte, 128 * sizeof(infer_t) + 256> memory1;
  MemoryBuffer buf1( std::span{memory1} );
  auto allocator1 = buf1.get_allocator<infer_t, 32>();

  std::array<std::byte, 128 * sizeof(infer_t) + 256> memory2;
  MemoryBuffer buf2( std::span{memory2} );
  auto allocator2 = buf2.get_allocator<infer_t, 32>();

  inferTensor<128, 1> avgpool_out( allocator1 );
  ...
  inferTensorView<128> new_view{ std::span< infer_t, 128 >( avgpool_out.data(), 128 ) };
  inferTensorView<128, 128> linear_mat_weights_1{ std::span<infer_t, 128 * 128>( linear_mat_1, 128 * 128 ) };
  inferTensor<128> linear_out_1( allocator2 );

  blas::gemv(linear_mat_weights_1, new_view, linear_out_1);

  inferTensorView<128> linear_add_weights_1{ std::span<infer_t, 128>( linear_add_1, 128 ) };

  in_place_eval( relu( linear_out_1.as_view() + linear_add_weights_1), linear_out_1 );

  inferTensorView<11, 128> linear_mat_weights_2{ std::span<infer_t, 11 * 128>( linear_mat_2, 11 * 128 ) };

  blas::gemv( linear_mat_weights_2, linear_out_1, out );

  inferTensorView<11> final_add{ std::span<infer_t, 11>( linear_add_2, 11 ) };
...
int main(int argc, char** argv) {
  ...
  static constexpr size_t INPUT_OUTPUT_ALLOC_BYTES = ( 3 * 128 + 128 * 32 ) * sizeof(infer_t) + 256;
  MemoryBuffer buf( INPUT_OUTPUT_ALLOC_BYTES ); // Alloc enough for input/output tensor plus some wiggle room
  auto alloc = buf.get_allocator<infer_t, 32>();

  inferTensor<3, 128> input(alloc);
  inferTensor<128, 32> output(alloc);
  ...
  for (;;) {
    ...
    try {
      ...
      for (unsigned int i = 0; i < batch_len; ++i) {
        conn.recv( std::span<std::byte>( reinterpret_cast<std::byte*>(input.data()), 3 * 128 * sizeof(infer_t)) );

        FeatureExtractor( input, output );

        std::array<infer_t, 11> final_buf;

        inferTensorView<11> final{ std::span( final_buf ) };

        ClassifierHead( output, final );

        unsigned char argmax = 0;
        for (unsigned char cls = 1; cls < 11; ++cls) {
          if (final[cls] > final[argmax]) argmax = cls;
        }
